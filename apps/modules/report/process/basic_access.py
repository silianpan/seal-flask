...
def get_post_access():
    ...
    data["7_total"] = mdb_web.db.post.find(
        {"issue_time": {"$gte": s_time}}).count(True)
    s_time = (now_time - 86400 * 29) - now_time % 86400
    data["30_total"] = mdb_web.db.post.find(
        {"issue_time": {"$gte": s_time}}).count(True)

    data["unaudited"] = mdb_web.db.post.find(
        {
            "audited": 0,
            'is_delete': 0,
            'issued': 1,
...
def get_comment_access():
    ...
    data["7_total"] = mdb_web.db.comment.find(
        {"issue_time": {"$gte": s_time}}).count(True)
    s_time = (now_time - 86400 * 29) - now_time % 86400
    data["30_total"] = mdb_web.db.comment.find(
        {"issue_time": {"$gte": s_time}}).count(True)

    data["unaudited"] = mdb_web.db.comment.find(
        {
            "audited": 0,
            'is_delete': 0,
            'issued': 1,
...
def get_user_access():
    ...
    now_time = time.time()
    s_time = (now_time - 86400 * 6) - now_time % 86400
    data["7_total"] = mdb_user.db.user.find(
        {"create_at": {"$gte": s_time}}).count(True)
    s_time = (now_time - 86400 * 29) - now_time % 86400
    data["30_total"] = mdb_user.db.user.find(
        {"create_at": {"$gte": s_time}}).count(True)
...
def get_message():
    ...
    data["email_abnormal"] = mdb_sys.db.sys_message.find(
        {"status": "abnormal", "type": "email"}).count(True)
    data["email_error"] = mdb_sys.db.sys_message.find(
        {"status": "error", "type": "email"}).count(True)

    data["sms_error"] = mdb_sys.db.sys_message.find(
        {"status": "error", "type": "sms"}).count(True)
    data["sms_abnormal"] = mdb_sys.db.sys_message.find(
...
def get_plugin():
    ...
    data["plugin_error"] = mdb_sys.db.plugin.find(
        {"error": {"$nin": ["", 0, False, None]}}).count(True)
    data["plugin_active"] = mdb_sys.db.plugin.find(
...
def get_media():
    ...
    now_time = time.time()
    s_time = (now_time - 86400 * 6) - now_time % 86400
    data["7_total"] = mdb_web.db.media.find(
        {"time": {"$gte": s_time}}).count(True)
    s_time = (now_time - 86400 * 29) - now_time % 86400
    data["30_total"] = mdb_web.db.media.find(
        {"time": {"$gte": s_time}}).count(True)
...
def get_inform_data():
    ...
    now_time = time.time()
    s_time = (now_time - 86400 * 6) - now_time % 86400
    data["7_post"] = mdb_web.db.post.find(
        {"inform.update_time": {"$gte": s_time}}).count(True)
    data["7_comment"] = mdb_web.db.comment.find(
        {"inform.update_time": {"$gte": s_time}}).count(True)
    data["7_user"] = mdb_user.db.user.find(
        {"inform.update_time": {"$gte": s_time}}).count(True)
    data["7_media"] = mdb_web.db.media.find(
        {"inform.update_time": {"$gte": s_time}}).count(True)

    s_time = now_time - now_time % 86400
    data["1_post"] = mdb_web.db.post.find(
        {"inform.update_time": {"$gte": s_time}}).count(True)
    data["1_comment"] = mdb_web.db.comment.find(
        {"inform.update_time": {"$gte": s_time}}).count(True)
    data["1_user"] = mdb_user.db.user.find(
        {"inform.update_time": {"$gte": s_time}}).count(True)
    data["1_media"] = mdb_web.db.media.find(
        {"inform.update_time": {"$gte": s_time}}).count(True)

    s_time = now_time - 3600 * 8
    data["8h_post"] = mdb_web.db.post.find(
        {"inform.update_time": {"$gte": s_time}}).count(True)
    data["8h_comment"] = mdb_web.db.comment.find(
        {"inform.update_time": {"$gte": s_time}}).count(True)
    data["8h_user"] = mdb_user.db.user.find(
        {"inform.update_time": {"$gte": s_time}}).count(True)
    data["8h_media"] = mdb_web.db.media.find(
        {"inform.update_time": {"$gte": s_time}}).count(True)

    s_time = now_time - 3600 * 3
    data["3h_post"] = mdb_web.db.post.find(
        {"inform.update_time": {"$gte": s_time}}).count(True)
    data["3h_comment"] = mdb_web.db.comment.find(
        {"inform.update_time": {"$gte": s_time}}).count(True)
    data["3h_user"] = mdb_user.db.user.find(
        {"inform.update_time": {"$gte": s_time}}).count(True)
    data["3h_media"] = mdb_web.db.media.find(
        {"inform.update_time": {"$gte": s_time}}).count(True)
